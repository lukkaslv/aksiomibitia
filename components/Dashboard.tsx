    ...
    <div className="space-y-8 lg:space-y-12 animate-in fade-in slide-in-from-top-4 duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
        <div>
          ...
          <p className="text-xs lg:text-base text-gray-400 font-light italic">Карта вашей внутренней трансформации.</p>
        </div>
        <button 
          onClick={onClose}
          className="text-[9px] font-bold uppercase tracking-widest text-gray-400 hover:text-black transition-colors border-b border-gray-100 pb-1"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 lg:gap-6">
        <div className="p-6 lg:p-8 rounded-2xl lg:rounded-3xl bg-white border border-gray-100 shadow-sm">
          ...
          <div className="text-[9px] font-bold uppercase tracking-widest text-gray-400">Прогресс</div>
        ...
        <div className="p-6 lg:p-8 rounded-2xl lg:rounded-3xl bg-white border border-gray-100 shadow-sm">
          ...
          <div className="text-[9px] font-bold uppercase tracking-widest text-gray-400">Постигнуто</div>
        </div>
        <div className="p-6 lg:p-8 rounded-2xl lg:rounded-3xl bg-white border border-gray-100 shadow-sm">
          <div className="text-2xl lg:text-4xl font-light text-gray-900 mb-1">{completedLevels.length}</div>
          <div className="text-[9px] font-bold uppercase tracking-widest text-gray-400">Уровней пройдено</div>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-12">
        <section>
          <h3 className="text-[9px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-4 lg:mb-6">Архив инсайтов</h3>
          <div className="space-y-3">
            {progress.insights.length === 0 ? (
              <div className="py-12 text-center border border-dashed border-gray-200 rounded-2xl">
                <p className="text-xs text-gray-300 italic">Пока пусто. Время собирать камни...</p>
              </div>
            ) : (
              progress.insights.map((insight, idx) => (
                <div key={idx} className="p-5 rounded-2xl border border-gray-100 bg-white shadow-sm">
                  <div className="flex justify-between mb-2">
                    <span className="text-[8px] font-mono text-gray-300 uppercase">{insight.id}</span>
                    <span className="text-[8px] text-gray-300 font-medium">{insight.date}</span>
                  </div>
                  <p className="text-sm text-gray-700 leading-relaxed font-light italic">"{insight.text}"</p>
                </div>
              )).reverse()
            )}
          </div>
        ...
        <section>
          <h3 className="text-[9px] font-bold uppercase tracking-[0.2em] text-gray-400 mb-4 lg:mb-6">Геометрия духа</h3>
            ...
            {LEVELS.map(level => {
              ...
              return (
                <div key={level.id} className="text-center">
                  <div className={`aspect-square rounded-xl lg:rounded-2xl border flex flex-col items-center justify-center transition-all duration-500 ${
                    isDone ? 'bg-black border-black text-white shadow-lg' : levelProgress > 0 ? 'bg-white border-gray-300 text-black' : 'bg-gray-50 border-gray-100 text-gray-300'
                  }`}>
